...
def image_transformation(data_dir,train_dir,valid_dir,test_dir):
    ...
    image_datasets_train = datasets.ImageFolder(
        train_dir, transform=data_transforms_train)
    image_datasets_valid = datasets.ImageFolder(
        valid_dir, transform=data_transforms_test)
    image_datasets_test = datasets.ImageFolder(
        test_dir, transform=data_transforms_test)

    dataloaders_train = torch.utils.data.DataLoader(
        image_datasets_train, batch_size=64, shuffle=True)
    dataloaders_valid = torch.utils.data.DataLoader(
        image_datasets_valid, batch_size=64)
...
train(model, epochs, learning_rate, criterion, optimizer,
      dataloaders_train, dataloaders_valid, 'GPU')
...
top_p, top_class, top_class_name = predict(
    "./flowers/test/10/image_07090.jpg", model, topk=5)
print(top_p)
# print(top_class)
